// GameAutomation/Application/SkillEngine.cs
// Phase 8 C4: 原 static class Skill (1809 行) → 实例化 SkillEngine。
// - 可变状态字段（锁字典 / 技能状态字典 / 时间字典 / 检测点缓存 / 重复按键执行间隔阈值）改实例字段。
// - 纯只读查找表（技能4/5/6、ColorMatcher、StringBuilderPool*、ColorConfig 等）保留 static。
...
// Heroes/ 86 文件 331 处 `Skill.X` 调用通过 Games/Dota2/Skill.cs facade 转发到 Common.SkillEngine，C7 替换调用方 → D1 删桥。
//#define 输出技能信息
#if DOTA2
...
namespace Dota2Simulator.GameAutomation.Application
{
    public sealed class SkillEngine
    {
        ...
        // C4: 技能类型 enum 保留在 Games/Dota2/Skill.cs facade（被外部 Heroes/ 引用 = Skill.技能类型）。
        // 本类内部用 `Skill.技能类型`（跨类引用 facade 嵌套类型），等 C7 + D1 删桥后再决定是否上移到 Domain。

        // Phase 8 C3: 原 static int 字段迁 HeroAggregate.SkillCount（跨 BC SSOT），同类内 10 处访问统一走 _aggregate 路径。
        ...
        public async Task<bool> 设置当前技能数量()
        {
            ...
            if (i != 0)
            {
                ...
                // 另一种是技能改变了
                if (_aggregate.SkillCount != i)
                {
                    _aggregate.SkillCount = i;
        ...
        // 使用缓存的检测配置（C4: static → instance，单实例 SkillEngine 等价）
        private readonly ConcurrentDictionary<(技能信息, int), 检测点[]> _检测点缓存 = new();
        ...
        private 检测点[] 获取检测点配置(技能信息 技能, int i)
        {
        ...
        private void 记录技能释放信息(Keys s1, Keys 技能位置)
        {
        ...
        private void 更新释放前颜色(Keys 技能位置, Color 当前获取颜色)
        {
        ...
        private readonly Dictionary<Keys, object> 锁字典 = new()
        {
            { Keys.Q, new object() },
            { Keys.W, new object() },
            { Keys.E, new object() },
        ...
        private readonly Dictionary<Keys, 技能状态> 技能状态字典 = new()
        {
            { Keys.Q, new 技能状态() },
            { Keys.W, new 技能状态() },
            { Keys.E, new 技能状态() },
        ...
        private bool 获取技能释放状态(Keys 技能位置, out bool 释放中判断, out bool 已释放变色判断, out Color 释放前Color, out Color 释放后Color)
        {
            lock (锁字典[技能位置])
            {
        ...
        private void 更新释放判断和颜色(Keys 技能位置, bool 释放中判断, bool 已释放变色判断, Color 释放前Color, Color 释放后Color)
        {
            lock (锁字典[技能位置])
            {
                if (技能状态字典.TryGetValue(技能位置, out 技能状态 技能状态))
        ...
        private void 重置指定技能判断(Keys key)
        {
        ...
        // 存储每个技能的释放开始时间
        private readonly Dictionary<Keys, DateTime> _技能释放开始时间 = new Dictionary<Keys, DateTime>();
        private readonly Lock _时间锁 = new();
        ...
        private DateTime 获取或设置释放开始时间(Keys 技能位置, bool 当前释放中)
        {
            lock (_时间锁)
            {
        ...
        private void 设置释放开始时间(Keys 技能位置, DateTime 开始时间)
        {
            lock (_时间锁)
            {
                _技能释放开始时间[技能位置] = 开始时间;
        ...
        /// </summary>
        /// <param name="技能位置">技能按键位置</param>
        private void 清除释放开始时间(Keys 技能位置)
        {
            lock (_时间锁)
            {
        ...
        private double 获取释放持续时间(Keys 技能位置)
        {
            lock (_时间锁)
            {
                if (_技能释放开始时间.TryGetValue(技能位置, out DateTime value))
        ...
        private bool 处理技能释放(Keys 技能位置, Color 当前释放颜色)
        {
        ...
        public async Task<bool> 主动技能已就绪后续(Keys skill, Action afterAction)
        {
            // 检查是否在时间间隔内
        ...
        /// <summary>
        ///     懒加载字典（C4: static → instance）
        /// </summary>
        private readonly Lazy<ConcurrentDictionary<Keys, DateTime>> _技能执行时间 =
        new Lazy<ConcurrentDictionary<Keys, DateTime>>(() => new ConcurrentDictionary<Keys, DateTime>());

        /// <summary>
        ///     获取对应Key的时间
        /// </summary>
        private ConcurrentDictionary<Keys, DateTime> 技能执行时间 => _技能执行时间.Value;

        /// <summary>
        ///     循环使用技能等待时间（C4: static → instance；facade Skill.重复按键执行间隔阈值 转发 property 调本字段）
        /// </summary>
        public int 重复按键执行间隔阈值 = 100;

        public void 清除技能时间记录(Keys skill)
        {
        ...
        /// <returns></returns>
        public async Task<bool> 技能通用判断(Keys 技能键, int 判断模式, bool 是否接按键 = true, Keys 要接的按键 = Keys.A,
            int 判断成功后延时 = 0)
        {
            void 技能后续动作()
            {
            ...
            Func<Task<bool>> 使用技能 = 判断模式 switch
            {
                ...
                2 => () => 主动技能已就绪后续(技能键, () => { _input.Press(VirtualKey.From(技能键)); }),
        ...
        /// <returns></returns>
        public async Task<bool> 释放技能后替换图标技能后续(Keys key, Func<int> 获取全局步骤, Action<int> 设置全局步骤)
        {
            int 全局步骤 = 获取全局步骤();
            ...
            switch (全局步骤)
            {
                case 1:
                    ...
                    {
                        设置全局步骤(0);
                        _aggregate.LegSwap.配置.修改配置(key, true);
                    }).ConfigureAwait(true);
                default:
                    _ = await 主动技能释放后续(key, () =>
                    {
                        设置全局步骤(1);
                        通用技能后续动作();
                        _aggregate.LegSwap.配置.修改配置(key, false);
                    }).ConfigureAwait(true);
        ...
        /// </summary>
        /// <param name="是否接A"></param>
        public void 通用技能后续动作(bool 是否接按键 = true, Keys 要接的按键 = Keys.A, int 等待的延迟 = 0, bool 是否保持假腿 = true)
        {
            _ = Task.Run(() =>
            {
                Common.Delay(等待的延迟);
                ...
                if (是否接按键)
                {
                    ...
                    _input.Press(VirtualKey.From(要接的按键));
                }
                else
                {
                    _input.MouseClick(MouseButton.Right);
        ...
        public void 后续切回假腿(bool 是否接按键 = true, Keys 要接的按键 = Keys.A, int 等待的延迟 = 0)
        {
            _ = Task.Run(() =>
            {
                Common.Delay(等待的延迟);
        ...
        public Task 测试方法(int x, int y)
        {
            if (_ui is null) return Task.CompletedTask;
            ...
            int 延迟 = 0;
            _ui.Invoke(() => { 延迟 = int.Parse(_ui.GetText(UiField.状态抗性).Trim(), CultureInfo.InvariantCulture); });
            cts.CancelAfter(延迟);
            ...
            {
                // 将结果转换为所需格式
                _ui.SetText(UiField.X, string.Join("|", colors.Select(c => $"{c.R},{c.G},{c.B}")));
                _ui.SetText(UiField.Y, string.Join("|", longs.Select((c, index) => index == 0 ? c : c - longs[index - 1])));
            });
        ...
        public async Task 捕捉颜色()
        {
            ...
            _ui.Invoke(() =>
            {
                selectedKey = _ui.GetText(UiField.阵营).Trim();
                timeout = int.Parse(_ui.GetText(UiField.状态抗性).Trim(), CultureInfo.InvariantCulture);
            });
            ...
            _ui.Invoke(() =>
            {
                // 将结果转换为所需格式
                _ui.SetText(UiField.X, string.Join("|", colors.Select(c => $"{c.R},{c.G},{c.B}")));
                _ui.SetText(UiField.Y, string.Join("|", timestamps.Select((c, index) => index == 0 ? c : c - timestamps[index - 1])));
            });
