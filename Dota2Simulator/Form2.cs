...
namespace Dota2Simulator
{
    internal partial class Form2 : Form
    {
        ...
        {
            // 1. 获取初始条件状态
            bool[] 条件数组 = 条件配置.Select(配置 => 配置.获取条件()).ToArray();
        ...
        private async Task 测试方法(int x, int y)
        {
            ...
            cts.CancelAfter(int.Parse(tb_状态抗性.Text.Trim()));
        ...
        private async Task 捕捉颜色()
        {
            ...
            // 使用取消令牌控制循环寿命
            var timeout = int.Parse(tb_状态抗性.Text.Trim());
            ...
            // 将结果转换为所需格式
            tb_x.Text = string.Join("|", colors.Select(c => $"{c.R},{c.G},{c.B}"));
            tb_y.Text = string.Join("|", timestamps.Select(t => t.ToString()));

            Tts.Speak("完成");
        }

        // 辅助方法：获取颜色配置
        private IReadOnlyList<ColorConfig> GetColorConfigurations()
        {
            // 预定义所有需要捕获的颜色配置
            ...
            foreach (var key in new[] { Keys.Q, Keys.W, Keys.E, Keys.R })
            {
                ...
                {
                    // 添加所有颜色类型
                    configs.Add(new ColorConfig($"{key.ToString().ToLower()}{count}", key, count, 技能类型.释放变色));
                    configs.Add(new ColorConfig($"{key.ToString().ToLower()}{count}1", key, count, 技能类型.图标CD));
                    configs.Add(new ColorConfig($"{key.ToString().ToLower()}{count}2", key, count, 技能类型.QWERDF图标));
                    configs.Add(new ColorConfig($"{key.ToString().ToLower()}{count}3", key, count, 技能类型.法球));
                    configs.Add(new ColorConfig($"{key.ToString().ToLower()}{count}4", key, count, 技能类型.状态));
                    configs.Add(new ColorConfig($"{key.ToString().ToLower()}{count}5", key, count, 技能类型.被动技能存在));
                }
            }
        ...
        // 统一获取技能颜色的方法
        private Color 获取技能颜色(Keys skillKey, in 字节数组包含长宽 数组, int skillCount, 技能类型 colorType)
        {
            switch (colorType)
            {
                case 技能类型.释放变色:
                    return 获取技能释放判断颜色(skillKey, in 数组, skillCount);
                case 技能类型.图标CD:
                    return 获取技能进入CD判断颜色(skillKey, in 数组, skillCount);
                case 技能类型.QWERDF图标:
                    return 获取QWERDF颜色(skillKey, in 数组, skillCount);
                case 技能类型.法球:
                    return 获取法球颜色(skillKey, in 数组, skillCount);
                case 技能类型.状态:
                    return 获取状态颜色(skillKey, in 数组, skillCount);
                case 技能类型.被动技能存在:
                    return 获取被动颜色(skillKey, in 数组, skillCount);
                default:
                    return Color.Empty;
            }
        }
        ...
        private static class StringBuilderPool2
        {
            private static readonly ThreadLocal<StringBuilder> _pool = new ThreadLocal<StringBuilder>(() => new StringBuilder());
        ...
        public static int 获取当前技能数量(in 字节数组包含长宽 数组)
        {
            ...
            var 全部文字 = StringBuilderPool1.Get();
            ...
            for (int j = 0; j < 技能列表.Count; j++)
            {
                ...
                var 输出文字 = StringBuilderPool2.Get();
                输出文字.AppendLine($"\r\n当前技能数量{期望数量}");
