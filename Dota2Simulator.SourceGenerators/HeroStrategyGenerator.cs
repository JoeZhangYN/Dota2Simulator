...
public sealed class HeroStrategyGenerator : IIncrementalGenerator
{
    ...
    {
        // Phase 19G-2: SG emit `: HeroStrategyBase` 继承 — base 持 ports/ctor 模板 + OnActivate/OnKeyAsync 默认实现.
        // SG emit 仅: 继承声明 + base 调用 ctor + override Hero (字段 / OnActivate / OnKeyAsync body 全 base 接管).
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("// SSOT: [HeroStrategy] attribute on " + t.ClassName + "; SG = HeroStrategyGenerator (Phase 19G-2 终极抽象).");
        sb.AppendLine("// emit partial 提供: : HeroStrategyBase 继承 + base 调用 ctor + override HeroId Hero (业务侧 BuildPlan() 一行表达式).");
        ...
        sb.AppendLine("{");
        sb.AppendLine($"    public sealed partial class {t.ClassName} : HeroStrategyBase");
        sb.AppendLine("    {");

        // ctor 签名: base 调用 (普通 5 ports / RequiresUi 6 ports 含 ui)
        if (t.RequiresUi)
        {
            ...
            sb.AppendLine("            : base(input, vision, skill, item, main, ui) { }");
        }
        else
        {
            ...
            sb.AppendLine("            : base(input, vision, skill, item, main) { }");
        }
        sb.AppendLine();

        // HeroId Hero override (data SSOT 从 attribute 派生)
        sb.Append("        public override HeroId Hero => new(\"")
